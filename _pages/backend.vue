    ...
    <div class="max-w-7xl mx-auto px-12 h-full relative z-1">
      <div class="flex flex-col gap-12 leading-0 h-full lg:w-1/2">
        <div class="flex flex-col gap-5">
          ...
          <span class="text-3xl font-medium text-gray-500"> </span>
          <div v-for="claim in claims" :key="claim.id">
            {{ claim.email }} // {{ claim.flight.departure.iata }} ->
            {{ claim.flight.arrival.iata }}
            <button class="bg-primary-500 text-white py-5 px-3 rounded-md">
              send email
            </button>
          </div>
        </div>
